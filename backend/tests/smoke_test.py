...
check("consultor NO crea agente (403)", c.post("/api/agents/", headers=cons_h, json={"key": "x_test", "name": "X", "system_prompt": "p"}).status_code == 403)
na = c.post("/api/agents/", headers=admin_h, json={"key": "abap_forms", "name": "Forms", "description": "Smartforms/Adobe", "provider": "gemini", "system_prompt": "Eres experto en Smartforms y Adobe Forms."})
check("admin crea agente 201", na.status_code == 201)
check("key inválido rechazado (400)", c.post("/api/agents/", headers=admin_h, json={"key": "AB C!", "name": "X", "system_prompt": "p"}).status_code == 400)
check("no duplica agente del sistema (400)", c.post("/api/agents/", headers=admin_h, json={"key": "abap_ecc", "name": "X", "system_prompt": "p"}).status_code == 400)
check("agente nuevo aparece en lista", any(a["key"] == "abap_forms" for a in c.get("/api/agents/", headers=admin_h).json()))
check("edita agente custom", c.put("/api/agents/abap_forms", headers=admin_h, json={"temperature": 0.5}).status_code == 200)
check("no borra agente del sistema (400)", c.delete("/api/agents/abap_ecc", headers=admin_h).status_code == 400)
check("borra agente custom (200)", c.delete("/api/agents/abap_forms", headers=admin_h).status_code == 200)
